...
You can use the Kubernetes CLI, `kubectl`, to connect to the Kubernetes control plane and troubleshoot or debug problems with your pods and containers. The following `kubectl` commands are useful:
...
Start an interactive session or issue a specific command inside a running container to get information about the environment or explore potential problems. Specify the pod name and the namespace. For example:
...
Troubleshoot pods by creating interactive debugging sessions or temporary debugging containers. For example, the following command creates a debugging container that uses BusyBox command-line tools (such as `ls`, `ps`, and `sh`). The original container doesn't have access to a shell or debugging tools, but you can use these tools in the debugging container to troubleshoot your environment.
